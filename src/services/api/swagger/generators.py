...
class CustomSwaggerAutoSchema(SwaggerAutoSchema):
    def __init__(
        ...
        self.auth_permission_classes_tuple = tuple(schema_settings.BASE_AUTH_PERMISSION_CLASSES)
    ...
    def get_request_serializer(self):
        if hasattr(self.view, "serializer_class") and self.method in self.body_methods:
            request_serializer_class = self.view.serializer_class
            return request_serializer_class() if request_serializer_class is not None else None
    ...
    def _add_default_not_found_response(self, responses):
        ...
        if status.NOT_FOUND not in responses and re.search(r"/{\w+}/", self.path):
